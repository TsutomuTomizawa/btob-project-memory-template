...
- 公開 repository に出せない内容は、private repository または社内 repository で扱ってください。
...
- 公開前、push 前、10-15 回保存ごとに `scripts/memory lint` を使う。
- 新規クライアント・社内プロジェクト追加では、まず `scripts/memory add ...` で agent 向け prompt を生成し、その内容に従って `memory-add` を使う。
- ユーザーが「pushして」「プッシュしてください」と依頼した場合は、差分を確認し、通常は `scripts/memory publish "短い日本語のコミットメッセージ"` で commit / push する。公開テンプレートでは自動 PR / auto-merge は使わない。
- ユーザーが「pullして」「最新化して」「syncして」と依頼した場合は、未保存変更がないことを確認し、`scripts/memory sync` で `main` を `origin/main` に fast-forward する。
...
人間は主に Obsidian、commit 差分、GitHub Actions の結果で内容を確認します。client / internal 配下の `profile.md`、`sources.md`、`states/current.md` と top-level `raw/`、gitignore された `inbox/` メモは手動編集できます。`events/` は skill なしで直接書き換えないでください。
