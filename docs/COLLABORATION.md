...
このリポジトリは GitHub + PR + 必須 CI を前提にします。
...
- 日常の push 依頼では `scripts/memory publish` を使い、`main` direct push ではなく作業ブランチ push に変換し、PR/CI/auto-merge 完了後に main へ戻す。
- `AUTO_PR_TOKEN` secret が設定されている場合、作業ブランチへ push すると main 向け PR を自動作成する。
- PR では `memory-vault-ci` と `memory-role-guard` を必須にする。CODEOWNERS は責任範囲の目印として残すが、GitHub 上の必須 review 条件にはしない。
- GitHub の native auto-merge が有効な場合は、CI が通った後に自動 merge する。
- publish が待機できなかった場合や別端末で merge された場合は、`scripts/memory sync` で `main` に戻り、最新版を取り込み、merge 済みの作業ブランチを片付ける。
...
- 作業ブランチ push / main 向け PR チェック: GitHub Actions で基本動作と lint を確認し、merge を止める。
